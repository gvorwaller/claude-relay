...
// Consume AGY's stream-json without persisting prompts, reasoning, commands,
// tool arguments, or tool output. Only fixed activity categories and the final
// operator-facing response are retained.
const fs = require('fs');
const { spawn } = require('child_process');
const WebSocket = require('ws');
const { projectAgyEvent } = require('../delegate-activity');
...
child.stdout.on('data', chunk => {
  ...
  for (const line of lines) {
    ...
    if (event.event === 'result' && event.result && typeof event.result.response === 'string') {
      finalResult = event.result.response;
    }
    const projected = projectAgyEvent(event);
